...
// ── 좌표 없는 곳 안내 (2026-09-06) ────────────────────────────────────────
// 수집기가 "자기 시장에서 100km 넘게 떨어진 좌표"를 비우면서 목록 수와 마커 수가 갈린다.
// 그 차이를 화면이 말해야 하는데, **묶음 표시일 때는 말하면 안 된다** — 그때 mapPins 는
// 비어 있어(서버도 폴백도 상한 초과면 빈 배열) 계산하면 전부가 좌표 없음으로 나온다.
// 2026-09-06 에 실제로 "29,728곳은 위치를 확인하지 못해"라는 거짓 문장이 떴다.
{
  const h = HTML['merchants.html'];
  check(/mn-nocoord/.test(h), '좌표 없는 곳 안내 문구가 있다');
  check(/SNAP\.mapTruncated \? 0 :/.test(h),
    '묶음 표시일 때는 0으로 둔다 — 그 분기에서 세면 전부가 좌표 없음이 된다');
  const i = h.indexOf('곳 이하로 좁히면 개별 마커로 전환됩니다');
  const j = h.indexOf('CLUSTER_MODE = false; clusterSeq++; clearClusterMarkers();', i);
  check(i > 0 && j > i && !/noCoordNote/.test(h.slice(i, j)),
    '묶음 분기 문장에는 안내를 이어 붙이지 않는다');
  check(/lat != null && x\.lng != null/.test(h) || /x\.lat != null/.test(h),
    '좌표 있는 것만 세어 차이를 구한다(두 모드에서 같은 식)');
}
