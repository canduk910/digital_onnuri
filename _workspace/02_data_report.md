...
- **수정 내역 (2026-08-06, SSM)**: department-ssm 행을 writer 확정 재서술(03_content_spec.md S6 수정 이력)로 교체 — `examples`를 "직영점"으로 좁히고(policy 요건 ④-보강: 직영만 제외, 개인 가맹점주 점포는 가능), `check_point`에 예외 확인법 추가. `verdict: denied`·`basis: [R4]` 유지 (대상을 직영점으로 좁혔으므로 불가 판정이 정확, 예외는 check_point가 안내).
