...
## 2026-09-03 (5차) — 롯데ON 편입: 조회 18곳 · 비대상 4곳 · 색인 2곳 검증

검증 대상: 미커밋 작업트리 15파일 + 픽스처 2개. `data/online_platforms.json`·`config.js` **무변경**(롯데ON 은 template 을 계속 비워 두므로 데이터 변경이 없다 — dataVersion 미상향이 옳다).

**판정: 조건부 통과.** 코드·계약·테스트·렌더·원본 실측 전부 통과. 다만 **ADR-19 결정 대장이 코드와 어긋난 채 남아 있다**(F-1) — 한 줄 추가 후 커밋한다.

### 1. 테스트

| 항목 | 결과 | 기대 |
|---|---|---|
| `./gradlew cleanTest test` | **171건** · 실패 0 · 오류 0 · 스킵 1 | 171 ✓ |
| 스킵 1건 | `SelfTestLiveTest`(라이브 카나리아) | 설계대로 |
| `ProbeJudgeTest` / `SelfTestContractTest` | 29 → **31** / 11 → **12** | — |
| `test_survey_probe.js` | **154건** | 154 ✓ |
| `test_index_nightly.js` | **58건** (레시피 5→2 로 축소분 반영) | 보고값 |

### 2. 경계면 교차 비교

**(a) 사유 3종 — 정확히 일치, 죽은 키 0.**

| 사유 | 서버 `EXCLUSION` | 프론트 `REASON` | `REASON_LONG` |
|---|---|---|---|
| `bot-blocked` | 1곳(사이소) | 있음 | 있음 |
| `scope-first` | 2곳 | 있음 | 있음 |
| `no-static-search` | 1곳(인어교주) | 있음 | 있음 |

`EX_SCOPE_MIXED` 상수가 **삭제**됐고, 프론트 `REASON` 배지 맵도 9키(조회 실패 6 + 사유 3)로 정리됐다. 양쪽에 남은 `scope-mixed` 문자열은 **주석 3곳뿐**(서버 1 · 프론트 2)이며 살아 있는 키가 아니다.

**(b) 롯데ON 은 절대 `none` 을 내지 않는다 — 코드 경로로 확인.**
`ProbeJudge` 가 `NONE` 을 반환하는 지점은 전부 3곳이고, 롯데ON 은 셋 다 닫혀 있다.

| 경로 | 조건 | 롯데ON |
|---|---|---|
| `:154` 등급 A | `noneMarkersBound` 매치 | 목록 **비어 있음** |
| `:170` 등급 B | `noneMarkersPlain` 매치 | 목록 **비어 있음** |
| `:179` 토큰 0 | `hits==0 && !echoesQuery` | `echoesQuery=true` 라 **차단** |

두 갈래 응답이 각각 떨어지는 상태:

| 응답 | 크기 | 판정 경로 | status |
|---|---|---|---|
| 온누리 0건 `{"itemList":[],"total":0}` | 130B | API 하한 60자 통과 → 문구 없음 → 토큰 판정 차단 → 샘플 0 · hits 0 < 임계 | **unclear** |
| 무의미어·`q` 누락·빈 값 | 0B | `html.isBlank()`(`:41`) 또는 하한 60자(`:51`) | **unknown** |

회귀 테스트 `롯데ON은_없다고_말하지_않는다` 가 `status != NONE` · 두 목록이 빈 것 · `echoesQuery` · `canDecideAbsent=false` 를 한꺼번에 고정한다.

**(c) 카나리아 — absent 에서 빠지고 present 가 유일한 탐지 수단.**
`SelfTestService.canDecideAbsent(t) = !bound.isEmpty() || !plain.isEmpty() || !echoesQuery` → 롯데ON 은 **false**. `one()` 이 `expected = absent ? (canDecideAbsent ? NONE : "") : LIKELY` 라 absent 대조에서 자동으로 빠진다.
**present 기대치 `김치` 가 실제로 성립하는지 라이브로 확인했다**(아래 3절) — 60건 · 온누리 표식 100%. 필터가 깨지면 이 기대치가 무너져 카나리아가 잡는다.
`SelfTestContractTest` 에 **확정 수단이 없는 몰은 정확히 2곳(찬스·롯데ON)** 이라는 화이트리스트 검사가 신설돼, 다른 몰에 이 예외가 조용히 번지는 것을 막는다.

**(d) `Scope.ONNURI_SCOPE` 확인.** `MALL_WIDE` 아님 → 화면에 `온누리 범위 밖 포함` 배지가 붙지 않는다(그 배지는 지금 우체국쇼핑 하나뿐이라는 명세와 일치).

**(e) 링크 계약.** 롯데ON `search_url_template` = `""`, `url` = `https://s.lotteon.com/x8fcPe9vMOi?ch_dtl_no=1044428`(상생스토어). `searchUrlFor` 는 API 몰이라 조회 URL 로 폴백하지 않으므로 **JSON 이 링크로 나갈 길이 없다.** 계약 테스트가 "template 또는 url 중 하나"를 요구하며 통과.

**(f) 색인 2곳 ∩ 실시간 18곳 = ∅.** 레시피 `onnuri-noljang`·`tpirates`. **롯데ON 레시피 잔존 0** — `crawlLotteOn`·`pbf.lotteon.com`·`seltDpShop`·id 문자열 전부 사라졌다.

**(g) 쇼핑 22 = 18 + 4, 양방향 고아 0.**

### 3. 원본 실측 재현 (직접 curl · **반환 전건 기준**)

조회 URL 을 그대로 쳤다. 표본이 아니라 `itemList` 전건을 셌다.

| 질의 | status | 본문 | `itemList` | `total` | `pdName` 매치 | 온누리 표식 |
|---|---|---|---|---|---|---|
| 김치 | 200 | 219,649B | 60 | 1,165 | 60 | **60/60 (100%)** |
| 로봇청소기 | 200 | 114,923B | 31 | 31 | 31 | **31/31 (100%)** |
| zzqqxyw12345 | 200 | **0B** | — | — | — | — |

- 온누리 표식은 두 형태를 모두 셌다 — `emblemInfo.emblemName == "온누리"` 와 `additionalFlagList` 안의 `{"type":"onnuri"}`. **전건이 둘 다 갖는다.** 반례 0.
- `"pdName"` 매치 수가 `itemList` 수와 **정확히 같다**(60=60 · 31=31). 연관검색어·배너가 새는 자리가 없어 **앵커 없는 titlePattern 이 안전하다**는 주석이 실측으로 뒷받침된다.
- 코드 주석의 실측치(김치 220,870B·total 1,166)와 오늘 값(219,649B·1,165)의 차이는 몇 시간 사이 카탈로그 변동분이며 판정에 영향이 없다.
- present 카나리아 질의 `김치` 가 현재 60건 · 100% 온누리로 성립한다 — **(c)의 유일한 탐지 수단이 실제로 서 있다.**
...
|---|---|---|
| 조회 목록 | 18행 | 18 ✓ |
| 비대상 접힘 | `확인하지 않은 나머지 4곳` · 행 4 · 배지 4 | 4 ✓ |
| 사유 곳 수·순서 | `1곳 bot-blocked` → `2곳 scope-first` → `1곳 no-static-search` | **1/2/1 성격순** ✓ |
| 3종 문구 | `REASON_LONG` 축자 일치 | ✓ |
| 롯데ON `unclear` 표시 | `판정하지 못했습니다` — "없다"로 읽히는 문구 없음 | ✓ |
| 원시 키 노출 | 0 | ✓ |
| 색인 층·블록 순서 | `pb-head → pb-list → pb-index → pb-bridge → pb-more → pb-foot` | ✓ |
| 탭·다리 회귀 | 다리 `13곳` → 클릭 후 카드 **13** | ✓ |
| 모바일 390px | `scrollWidth 390 == clientWidth` | ✓ |
| pageerror | **0** | ✓ |
| 캐시버스트 | `online-probe.js?v=12` | ✓ |
...
|---|---|---|
| 온라인몰 active | 30 (쇼핑 22 + 배달 8) | 유지 |
| 검색 링크 보유 | **16** | 변동 없음 ✓(롯데ON template 은 계속 비어 있다) |
| 카탈로그 / taxonomy | 22 / 11 | 유지 |
| `index.html`·`build_index.py`·`data/`·`config.js`·`chat-widget.js` | 무변경 | ✓ |
| `verify_build.py`(D-F1) | 전체 통과(참고 실행) | ✓ |

### 6. 문서 정합

- `CLAUDE.md`·`03_content_spec` S19·`19_online_probe` 6-10-2·`17_backend_notes`·`DEPLOY.md` 는 전부 18곳·비대상 4곳·색인 2곳·카나리아 36건/일(18×2)로 갱신됐다. `03 spec` 은 곳 수를 적지 않는 원칙을 유지하고 `scope-mixed` 가 하루 만에 사라진 경위를 남겼다.
- **`_workspace/16_arch_decisions.md` ADR-19 만 갱신되지 않았다 → F-1.**
- **4차 게이트 F-1(robots 감시 공백) 해소를 확인했다** — `ROBOTS_WATCH_IDS` 가 조회 대상 **18곳을 전부** 덮는다(미감시 0). 4차 I-1(주석 낡음)도 해소됐다.
...
| # | 심각도 | 위치 | 기대 vs 실제 |
|---|---|---|---|
| **F-1** | **보통(문서)** | `_workspace/16_arch_decisions.md` ADR-19(:128 이 마지막 상태 기록) | 결정 대장이 **코드와 정면으로 어긋난다.** 대장은 아직 `롯데ON 은 보류` · `색인 대상은 3곳(놀장·인어교주·롯데ON)` · `비대상 5곳(… scope-mixed 1 …)` 이라고 적혀 있는데, 코드는 롯데ON 이 **18번째 조회 대상**이고 색인 2곳·비대상 4곳이며 `EX_SCOPE_MIXED` 는 삭제됐다. 다른 문서 5종은 전부 갱신됐고 **ADR 만 빠졌다.** 이 대장은 "왜 그렇게 정했나"의 근거지라, 다음 사람이 여기만 읽으면 이미 뒤집힌 판단(보류 사유 2가지)을 현재 정책으로 읽는다. 4차까지 매 회차 append 하던 것이 이번에 끊겼다 |
| I-1 | 정보(주석) | `online-probe.js:65~67` | 주석이 `(11번가·롯데ON·공영쇼핑은 scope-mixed 로, 현대홈쇼핑은 실시간 대상으로 빠졌다)` — 롯데ON 은 이제 조회 대상이고 `scope-mixed` 는 없다. **바로 위 블록(:58~63)은 이번에 갱신됐는데 이 줄만 남았다** |
| I-2 | 정보(관측) | `nightly_update.py` robots 감시 · 롯데ON | 감시에 넣은 것은 맞지만 **지금은 신호가 나오지 않는다.** 데이터 `url` 이 단축주소라 `s.lotteon.com/robots.txt` 가 **301 루프**(직접 확인: 5회 리다이렉트 후 `s.lotteon.com/www.lotteon.com`)로 끝난다. 코드 주석이 "잘못된 판정이 아니라 관측 못 함"이라고 스스로 밝혀 둔 상태다. 로그의 "변화 없음"을 "확인했다"로 읽지 않도록 추적이 필요하다 |
| I-3 | 정보(데이터) | `hyundai-home-shopping` | 4차 I-2 이월 — 템플릿이 값 없는 `&dispOrd` 로 끝난다. 이번 회차는 데이터 무변경이라 그대로다 |

### 9. 미검증(통과로 적지 않음)

- **라이브 카나리아 36건**(18×2): `SelfTestLiveTest` 는 스킵됐다. 롯데ON 은 absent 대조에서 빠지므로 **present(`김치` → likely + 샘플 ≥1) 한 건이 이 몰의 유일한 감시 지점**이다. 배포 후 반드시 그 한 건을 눈으로 확인할 것 — 3절에서 지금 성립함은 확인했으나 카나리아 경로로는 아직 돌지 않았다.
- **단계 F 실행**: playwright 미설치. 남은 레시피 2곳이 모두 브라우저형이라 이번엔 정적 대체 실행도 불가능했다(4차의 롯데ON 정적 경로가 사라졌다).
- **V8 색인 테이블 실 질의**: 로컬 Postgres 미기동.

---

## 2026-09-03 (6차) — robots 감시 개편(ADR-21) 검증

검증 대상: 미커밋 작업트리 9파일 + 신설 4파일. **검증 중 작업트리가 계속 움직였다** — `backend/DEPLOY.md`(13:35)·`backend/tools/nightly_update.py`(13:33)가 게이트 도중 수정됐다. 자바 소스는 그대로였고, 배치 관련 실측은 **최종본으로 재측정해 같은 결과**를 확인했다. 아래 문서 지적은 13:36 스냅샷 기준이다.

**판정: 조건부 통과.** 판정 정확성·경계면·아웃바운드·회귀 전부 통과했고, 내가 독립적으로 만든 케이스와 실측으로 라이브 판정을 재현했다. 다만 게이트 1항의 **크롤러 robots 테스트(33건)가 존재하지 않고**(F-1), 코드 주석 하나가 **판정을 뒤집는 방향으로 사실과 반대**다(F-2).

### 1. 테스트

| 항목 | 결과 | 기대 |
|---|---|---|
| `./gradlew cleanTest test` | **189건** · 실패 0 · 오류 0 · 스킵 1 | 189 ✓ |
| `RobotsRulesTest`(신설) | 13건 | — |
| `SelfTestContractTest` | 12 → **17건** | — |
| `test_survey_probe.js` | **154건** | 154 ✓ |
| `test_index_nightly.js` | **58건** | 58 ✓ |
| **크롤러 robots 순수 로직(33건)** | **존재하지 않음** | 33 ✗ → F-1 |

### 2. 판정 정확성 — 내가 만든 케이스 25건 (핵심)

`RobotsRules.java` 를 스크래치패드에 복사해 **독립 컴파일**하고 직접 작성한 케이스를 돌렸다(저장소 테스트와 무관).

| 축 | 케이스 | 결과 |
|---|---|---|
| UA 그룹 | 우리 토큰 > `*` / 우리를 안 가리키면 `*` / **더 긴 그룹이 이긴다** / 어느 그룹도 없으면 허용 / 키 대소문자 무시 | 5/5 |
| 최장 일치 | 긴 Allow > 짧은 Disallow / 긴 Disallow > 짧은 Allow / **동률이면 Allow** / 안 걸리면 허용 | 4/4 |
| 파싱 | 빈 `Disallow:` 무시 / 빈 값이 `Disallow: /` 를 덮지 않음 / 주석 제거 / 규칙 뒤 UA = 새 그룹 | 4/4 |
| `$`·와일드카드 | `Allow: /$` 는 루트만 / 하위 경로 불가 / **쿼리가 붙으면 불일치** / `/*.pdf$` / 비매칭 통과 / 중간 `*` | 6/6 |
| 실전 | 11번가 `plan` 허용·그 밖 차단 / **굿데이·인더마켓 조회경로(쿼리 포함) 차단** / 쿼리를 버리면 허용으로 뒤집힘(함정 재현) | 4/4 |
| 빈 파일 | null·빈 문자열 → 허용 | 2/2 |

**합계 25/25 통과.** 게이트가 지목한 두 자리가 모두 옳게 나온다.

**라이브 판정 독립 재현** — 조회 18곳의 호스트·경로를 `ProbeTargets` 에서 직접 뽑고, 18개 호스트의 `robots.txt` 를 내가 받아 위 파서로 판정했다.

| 결과 | 곳 수 | 몰 |
|---|---|---|
| **차단** | **5** | 굿데이·인더마켓·현대홈쇼핑·롯데ON(`Disallow: /`), 공영쇼핑(**`Disallow: /search/`**) |
| 허용 | 13 | 나머지 |
| 관측 실패 | 0 | — |

**보고된 "차단 5곳/18곳"과 정확히 일치한다.** 공영쇼핑은 경로 단위 규칙이라 **옛 한 줄 스캐너로는 잡히지 않던 자리**이고, 11번가는 조회 호스트 `apis.11st.co.kr` 의 robots 가 404라 허용으로 나온다.

### 3. 경계면 교차 비교

**(a) 계약 일치.** 앱 `SelfTestReport` 의 `probeEndpoints[]{platformId,host,path}` · `robotsUserAgent` · `robots[]{platformId,allowed,rule,group,error}` ↔ 배치 `_probe_robots_from_app` 이 읽는 키가 1:1. 키가 **호스트가 아니라 `platformId`** 라는 점도 양쪽 주석이 같이 밝힌다.

**(b) 중복 조회 0 — 실측.** `_robots_watch` 는 `probe_on` 이면 앱 판정을 옮기기만 하고 그 호스트를 다시 가져오지 않는다. 배치가 조회하는 `coarse_targets` 는 폴백(앱 불통 회차만) + 색인 호스트뿐이다. 조회 호스트 18개 ∩ 색인 호스트 3개 = **∅**.

**(c)·(d) 층별 조건 — `urlopen` 을 가로채 실측(외부 요청 0).**

| 조건 | 아웃바운드 | probe | coarse | 로그 |
|---|---|---|---|---|
| 조회 ON + 단계 F ON | **3건**(색인 호스트만) | 2 | 3 | 정상 |
| **킬스위치 OFF** + F ON | **3건**(색인 호스트만) | 0 | 3 | `kill-switch-off` |
| **둘 다 OFF** | **0건** | 0 | 0 | `kill-switch-off` + `단계 F 스킵으로 3곳 확인 안 함` |
| app-unreachable | 20건(폴백) | 0 | 20 | `app-unreachable` |
| no-admin-key | 20건(폴백) | 0 | 20 | `no-admin-key` |

사유 3종이 모두 나오고, 어느 경우에도 로그가 **`변화 없음이 아니라 확인하지 않았다`** 를 붙인다. 폴백 회차는 `11번가·5일장·롯데ON 은 실제 조회 호스트와 다른 사이트를 본다` 를 함께 찍어 관측점이 어긋남을 스스로 밝힌다(실제로 `search.11st.co.kr`·`www.onnuri5.com`·`s.lotteon.com` 을 본다).

**(e) 등급 구분.** 색인·폴백 결과에 `grade: "coarse"` 와 `owners` 가 붙고, 리포트에 `grades` 설명이 함께 저장된다. 앱 판정은 `grade: "app"`. 델타 비교도 같은 등급끼리만 한다.

**(f) 404·410.** 배치 `_robots_scan` 은 404·410 만 `blocked_all=False`(파일 없음)로, 나머지 HTTP 코드는 `error` 로 남긴다 — 301 루프·403 을 "허용"으로 적지 않는다. **앱 `fetchRobots` 는 404 만** 빈 규칙으로 보고 410 은 예외로 처리한다 → I-2(안전한 방향이나 두 등급이 다르다).

### 4. 아웃바운드 총량 (평시 하루)

| 층 | 건수 | 행선지 |
|---|---|---|
| 카나리아 | **36** | 조회 18곳 × 2질의(몰 검색 엔드포인트) |
| 앱 robots | **18** | 조회 호스트 18개 × 1(호스트 단위 1회) |
| 배치 coarse robots | **3** | 색인 호스트 `mall.noljang.co.kr`·`tpirates.com`·`pub-api.tpirates.com` |
| **합계** | **57** | **중복 0**(두 집합이 서로소) |

ADR-21 은 `하루 18건 안쪽(카나리아 36건과 합쳐 54건)` 이라 적었다 — 배치의 색인 호스트 3건이 빠져 있다(I-1).

### 5. 회귀

| 항목 | 값 | 판정 |
|---|---|---|
| 조회 대상 / 비대상 | 18 / 4(bot-blocked 1 · scope-first 2 · no-static-search 1) | 불변 ✓ |
| 색인 레시피 | 2 | 불변 ✓ |
| 온라인몰 30 · 쇼핑 22 · 검색 링크 16 | 그대로 | ✓ |
| `online.html`·`online-probe.js`·`chat-widget.js`·`data/`·`config.js`·`index.html`·`build_index.py` | **무변경** | ✓ |
| `verify_build.py`(D-F1) | 전체 통과 | ✓ |

### 6. 문서

ADR-21 이 코드와 맞는다 — 파서 위치, 셀프테스트 3필드, 기준선 상수 삭제 후 어제 리포트 비교, 킬스위치 연동, `404·410 만 파일 없음`, 그리고 **기준선 "차단 5곳"까지 내 재현과 일치**한다. `구현 중 발견 ①`(쿼리를 안 붙이면 굿데이·인더마켓이 허용으로 뒤집힌다)도 내가 독립적으로 재현했다.
DEPLOY.md 의 robots 절(:512~576)은 최종본 기준으로 코드와 맞는다(호스트 출처 3갈래·`platformId` 키·사유 4종·등급 2종·리포트 파일). 예시 로그의 `조회 대상 7곳` 은 실제 18곳과 다르나 예시 표기다(I-3).
...
| # | 심각도 | 위치 | 기대 vs 실제 |
|---|---|---|---|
| **F-1** | **보통(테스트)** | 크롤러 robots 순수 로직 테스트 | 게이트가 기대한 **33건이 존재하지 않는다.** 전수 탐색 결과 robots 전용 테스트 파일이 없고 `test_index_nightly.js` 에도 robots 검사 그룹이 없다(58건은 5차와 동일). ADR-21 이 파싱을 앱으로 옮겼으니 크롤러 쪽 테스트가 **불필요해진 것일 수 있다** — 그렇다면 게이트 기대치를 정정하면 되고, 필요한데 빠진 것이면 작성해야 한다. 어느 쪽인지 결정이 필요하다. 판정 로직 자체는 `RobotsRulesTest` 13건 + 내 독립 케이스 25건으로 덮여 있어 **관측 공백은 없다** |
| **F-2** | **보통(주석)** | `ProbeEndpoint.java` 클래스 javadoc | 주석이 `질의어는 담지 않는다 — 경로만 싣고 쿼리스트링은 버린다` 라고 말하는데, `SelfTestService.endpoints()` 는 **쿼리를 붙인다**(그 자리에 왜 붙이는지 긴 주석까지 있다). 이 차이가 판정을 뒤집는다 — 내 케이스로 재현했다: 쿼리를 버리면 굿데이·인더마켓이 `Allow: /$` 에 걸려 **차단 → 허용**으로 뒤집힌다. 즉 record 주석을 믿고 "정리"하는 사람이 감시를 **허용 방향으로** 조용히 망가뜨릴 수 있다. 질의어 자리는 이미 고정 토큰 `Q` 라 개인정보 우려도 없다 |
| I-1 | 정보(문서) | `16_arch_decisions.md` ADR-21 결과 절 | 아웃바운드를 `54건` 으로 적었으나 실측 **57건**(카나리아 36 + 앱 robots 18 + 배치 색인 robots 3). 배치 몫 3건이 빠졌다 |
| I-2 | 정보(코드) | `ProbeFetcher.fetchRobots` | 404 만 빈 규칙으로 보고 **410 은 예외**로 떨어진다. 배치는 404·410 둘 다 "파일 없음"으로 본다. DEPLOY.md 도 `404·410 제외`라 적는다. 안전한 방향(모르면 차단으로 세지 않고 error)이지만 두 등급의 규칙이 다르다 |
| I-3 | 정보(문서) | `DEPLOY.md` robots 예시 로그 | 예시가 `조회 대상 7곳(앱 판정)` — 실제는 18곳. 예시임이 분명하나 곳 수가 실제와 달라 오독 소지 |

### 9. 미검증(통과로 적지 않음)

- **라이브 셀프테스트 응답**: 배포 후에만 확인할 수 있다. 내가 검증한 것은 **같은 파서로 같은 입력에 대해 같은 답이 나온다**는 것(오프라인 재현)이고, 실제 앱이 `probeEndpoints`·`robots[]` 를 계약대로 실어 보내는지는 배포 후 `GET /api/online/search/selftest` 1회로 확인해야 한다.
- **배치 단계 E 실서버 1회전**: 앱을 호출하는 경로는 스텁으로만 돌렸다. 첫 회차는 비교할 어제 리포트가 없어 델타가 비는 것이 정상이다.
- **카나리아 36건**: `SelfTestLiveTest` 는 평소대로 스킵됐다.

### 6차 재검증 (확정본 기준, 13:41 스냅샷)

게이트 지시 뒤 crawler 가 파일을 확정했다는 통지를 받고 전 항목을 다시 돌렸다. **앞 절의 지적 F-1·F-2·I-1·I-2 는 전부 해소를 확인했다.**
...
| 항목 | 앞 절 | 확정본 |
|---|---|---|
| `./gradlew cleanTest test` | 189 | **191** · 실패 0 · 스킵 1 |
| `RobotsRulesTest` | 13 | **15** (게이트 도중 2건 추가) |
| `SelfTestContractTest` | 17 | 17 (robots 관련 신규 5) |
| `test_survey_probe.js` / `test_index_nightly.js` | 154 / 58 | 154 / 58 |

**robots 순수 로직 = `RobotsRulesTest` 15 + `SelfTestContractTest` 신규 5 = 20건.** 개정 기대치 18보다 2건 많은데, 파일이 게이트 도중 계속 자란 결과다(내 첫 집계 시점 13 → 확정 15). `RobotsRules.java` 본체는 13:15:09 이후 바뀌지 않았고, **내 독립 케이스 25건을 확정본에 다시 돌려 25/25 통과**를 재확인했다.

**2) 판정 로직이 한 곳에만 있는가 — 그렇다(조건부로 정확히 말하면)**

- 조회 대상용 파서는 배치에서 **완전히 사라졌다.** `RobotsRules` 하나뿐이다.
- 배치에 남은 `_robots_scan` 은 **`coarse` 전용**이고 조회 호스트를 절대 보지 않는다.
- **집합이 서로소다** — 조회 호스트 18개 ∩ 색인 호스트 3개 = ∅(실측). 같은 호스트를 두 로직이 판정하는 일이 구조적으로 생길 수 없고, 결과에는 `grade` 가 붙는다.

**3) 상수**

| 상수 | 상태 |
|---|---|
| `ROBOTS_WATCH_IDS` | **0건 — 삭제됨** ✓ |
| `ROBOTS_BLOCKED_AT_SURVEY` | 살아 있는 상수 **없음**. 남은 1건은 "왜 없앴는지"를 적은 **주석 안 역사 기록** ✓ |
| `ROBOTS_FALLBACK_IDS` | 폴백 전용으로 유일하게 남음 ✓ (20개 = 조회 18 + 놀장·인어교주) |

**4) 키 설계 — 근거는 현재 "가정"이다**

조회 대상 18곳의 **고유 호스트가 정확히 18개**이고, **한 호스트에 조회 대상이 둘 이상인 사례는 0개**다. 즉 crawler 가 든 근거("한 호스트라도 경로가 다르면 답이 다를 수 있다")에 해당하는 실물이 지금은 없다.
다만 **반대 방향은 실재한다** — 인어교주해적단이 `tpirates.com` + `pub-api.tpirates.com` 두 호스트를 쓰고, 그건 호스트 키인 `coarse` 층이 받는다. `platformId` 키는 호스트 키의 상위 집합이라 **지금은 무해하고 더 정밀한 쪽**이며, 몰이 늘어 한 호스트를 나눠 쓰는 날 그대로 맞다. 근거가 관측이 아니라 설계 예측이라는 점만 기록한다.

**5) `error` 행이 차단 집계에서 빠지는가 — 양쪽 다 빠진다**

- 배치 `nightly_update.py:845` — `blocked = ... if not r["allowed"] and not r["error"]`, `failed` 는 따로 세어 `관측 실패: … (차단이 아니라 못 읽은 것)` 으로 별도 로그.
- 앱 `SelfTestService:157` — `filter(c -> !c.allowed() && c.error() == null).count()`.

차단으로도 허용으로도 세지 않고 제3의 표기로 남는다.

**6) 델타가 같은 등급끼리만 비교하는가 — 그렇다**

`_probe_delta(prev_doc["probe"], probe)` 는 app 끼리, `_robots_delta` 는 `prev_doc["coarse"]` 를 **`grade == "coarse"` 로 걸러** coarse 끼리 비교한다(`:841`). 교차 비교가 코드상 불가능해 폴백 회차의 거짓 변화가 나지 않는다.

**7) 앞 절 지적의 처리**

| # | 상태 |
|---|---|
| F-1 (33건 테스트 부재) | **해소** — 기대치가 18로 정정됐고, 줄어든 자리가 앱으로 옮겨진 것을 확인했다(위 2항) |
| F-2 (`ProbeEndpoint` 주석이 사실과 반대) | **해소** — 주석이 재작성돼 `path 에는 쿼리스트링이 붙는다. 지우지 마라 — 판정이 뒤집힌다` 로 바뀌었고 고정 테스트 이름까지 가리킨다. 더해 `SelfTestContractTest.경로에_쿼리를_붙여야_robots_판정이_맞는다` 가 뒤집힘을 실제로 단언한다 |
| I-1 (아웃바운드 54) | **해소** — ADR 이 `평시 하루 57건(dev-qa 6차 실측)` 과 층별 내역으로 정정 |
| I-2 (앱이 410 미처리) | **해소** — `ProbeFetcher:79` 가 `status == 404 \|\| status == 410` 로 배치·문서와 일치 |
| I-3 (DEPLOY 예시 `7곳`) | **잔존** — `DEPLOY.md:566~567` 예시 로그가 아직 `조회 대상 7곳(앱 판정)`. 실제 18곳 |

**8) 새 지적 (정보)**

`nightly_update.py:68` 이 `이제 호스트의 출처는 셋이고 **어느 것도 손으로 적지 않는다**` 라고 적지만, 같은 파일 `:74` 의 `ROBOTS_FALLBACK_IDS` 는 **손으로 적은 20개 목록**이다. 지금 값은 정확하고(빠진 둘은 사이소·시장을방으로로 우리가 두드리지 않는 몰이라 제외가 맞다), 폴백은 앱 불통 회차에만 도는 열화 경로다. 다만 조회 대상이 늘 때 이 목록에 넣는 걸 잊으면 **그 열화 회차에서만 조용히 빠진다** — ADR 이 방금 없앤 상수 노후화 패턴과 같은 형태이므로 문구를 정확히 하거나 목록을 파생시키는 편이 낫다.

**9) 미검증(그대로)** — 라이브 셀프테스트 응답은 배포 후에만 확인 가능하다. 오프라인 재현까지가 내 범위다.
