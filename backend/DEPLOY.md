...
### ⚠ 단계 A 를 같은 날 두 번 돌리지 말 것 (2026-09-05 실측 · 2026-09-06 코드 가드)

배치를 손으로 다시 돌릴 일이 있으면 **`--skip-merchants` 를 붙인다.** 같은 날 두 번째
전수 수집은 공식 API 가 중간에 **HTTP 400** 을 낸다(격자 1,267지점을 하루에 두 번 훑는 셈).

**2026-09-06 부터 수집기가 스스로 막는다.** 같은 날짜 캐시가 있는데 `--refresh` 를 주면
종료 코드 **4** 로 멈추고, 조립만 다시 하는 길(`--refresh` 빼기)과 강행하는 길
(`--force-refresh`)을 함께 알려 준다. 문서만으로는 막지 못한다는 것이 실제로 드러났다 —
아래 사고를 낸 다음 날, 원인을 캐던 사람이 같은 자리를 그대로 밟을 뻔했다.

**차단은 그날로 끝나지 않는다.** 2026-09-05 09:32 의 두 번째 실행이 400 을 냈고,
**다음 날 00:30 배치가 1분 만에 같은 400 으로 죽었다.** 09-06 11시에 같은 호출을 해 보니
정상이었으므로 규격이 바뀐 것은 아니고 한동안 막혔던 것이다. 그날 라이브에는 중단 배너가
떴고 데이터는 09-05 수집분이 남았다(fail-open 이 지켰다).

그러면 이런 일이 벌어진다 — fail-open 이 설계대로 데이터를 지키고(기존 유지)
`app_meta.merchants_stale_since` 를 세우는데, **화면에는 오렌지 경고 배너가 뜬다**:

> ⚠ 자동 갱신이 멈춰 있습니다 — 온누리 공식 시스템이 …부터 가맹점 정보 자동 수집을 차단했습니다.

00:30 회차가 멀쩡히 성공했는데도 **이용자에게는 "수집이 막혔다"고 보인다.**
2026-09-05 에 실제로 그렇게 만들었고, 이렇게 지웠다:

```bash
docker exec onnuri-db psql -U "$DB_USER" -d "$DB_NAME" \
  -c "DELETE FROM app_meta WHERE k IN ('merchants_stale_since','merchants_stale_reason');"
curl -s https://api.koscomlabor.cloud/api/meta    # staleSince 가 null 로 돌아왔는지 확인
```

**플래그를 지우는 것은 그날 수집이 실제로 성공했을 때만이다.** 진짜로 막힌 상태에서
지우면 화면이 거짓말을 시작한다 — 그 배너는 2026-09-01 에 나흘간 아무도 모르고 지나간
일을 겪고 만든 것이다.
...
| 소요 | 약 22분 (0.7초 스로틀, 2026-09-01 실측) — 구 방식은 약 200초 |
| 커버리지 | 99.56% |

### 셀 하나가 실패해도 회차를 살린다 (2026-09-11)

2026-09-11 계측이 보여 준 것: 실패는 **격자 9번 한 지점**이었고 상대가 낸 것은
HTTP 500 · `resCode 9999 "시스템에 오류가 발생하였습니다."` 였다(차단 코드 9998 이 아니다).
그런데 종전 구조는 **그 한 셀 때문에 1,267 셀 전체가 죽어** 그날 수집이 통째로 날아갔다.

- 셀마다 **최대 3번 시도**(첫 시도 + 재시도 2회, 백오프 5초·15초).
- 세 번 다 실패하면 **그 셀만 건너뛴다.** 회차는 계속된다.
- **건너뛴 셀의 가맹점은 지우지 않는다** — 직전 회차 캐시에서 **그 셀 조회가 덮었을
  반경 2km 안**의 레코드를 그대로 이어받는다. (셀 '안'이 아니라 '조회 범위' 기준이다 —
  격자 색인 특성상 둘이 다르고, 멤버십으로 하면 **9,797곳 부류가 조용히 사라진다**.)
- 차단기 둘: **연속 10개** 셀이 실패하면 중단(셀 문제가 아니라 API 문제로 본다),
  **스킵이 격자의 5%**를 넘어도 중단. 둘 다 fail-open — 기존 데이터를 유지한다.
  차단기가 없으면 API 가 죽은 날 1,267 × 3 = **약 3,800 요청**이 나간다.

**로그에서 볼 것** (배치 로그에 그대로 찍힌다):

```
  ! 셀 스킵 9/1267 (35.050450,128.969167) — 3번 시도 실패, 이전 수집분 116건 유지 · HTTP Error 500
⚠ 스킵한 셀 1개 / 격자 1267개 — 그 안의 가맹점 116건은 2026-09-07 수집분을 그대로 유지했다
⚠ 그중 1개는 직전 회차에도 스킵된 셀이다 — 이어받기로 버티는 중이라 …
```

마지막 줄이 뜨면 **이어받기가 며칠째 버티는 중**이라는 뜻이다. 이어받기는 하루짜리
응급처치이지 해결이 아니다 — 같은 셀이 계속 스킵되면 그 안의 가맹점은 확인되지 않은 채
'오늘 수집분'으로 실린다. 좌표는 회차 리포트(`~/onnuri_batch/survey/merchant-collect-*.json`)의
`skipped` 에 있다.

**스로틀 0.7초 → 2.0초(2026-09-10)** — 09-08~10 사흘 연속 재수집이 HTTP 400 으로
죽었는데, 실패 시점이 매일 앞당겨졌다(411번째 → 211번째 → 약 10~15번째 요청). 요청
간격이 원인이라는 확증은 없지만 저희 쪽에서 바꿀 수 있는 유일한 변수라 사용자 결정으로
늘렸다. 전체 소요는 약 22분 → **49분 40초**로 늘어났다(2026-09-12 실측: 1,304 요청·2,980초·
요청당 2.27초. 예상치 49분과 거의 일치했다). 이 변경만으로 400 이 그치는지는 **다음 며칠의
배치 로그로만 확인된다** — 안 그치면 요청 간격이 원인이 아니라는 뜻이다.
