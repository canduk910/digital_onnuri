...
public class ChatService {
    ...
    String navigate(JsonNode a, Consumer<ChatEvents.NavigateAction> onAction) {   // 패키지 가시성 — ChatContractTest가 tab 검문을 고정
        ...
        // 2026-09-04: 종전에는 "카드를 눌러 이동할 수 있다고 안내할 것" 이었다 — **카드가 없을 때가 있다.**
        // 자동모드가 기본 ON 이라 그 경우 위젯이 카드를 만들지 않고 바로 이동하거나 제자리에서
        // 필터를 적용한다(chat-widget.js handleAction). 그런데 답변은 "아래 카드를 누르면"이라고
        // 말해, 이용자는 있지도 않은 카드를 찾는다. 화면 요소를 단정하지 않게 지시를 바꾼다.
        return "이동 요청을 화면에 전달했다. 이용자의 '자동모드' 설정에 따라 곧바로 이동하거나 확인 카드가 뜬다. "
                + "답변에서 '아래 카드를 누르면'처럼 화면 요소를 단정하지 말고 "
                + "'해당 화면에서 확인할 수 있다' 정도로 안내할 것.";
    }
    ...
    String systemPrompt() {   // 패키지 가시성 — ChatContractTest가 그림 우선 지시를 고정
        ...
        return """
                ...
                - 검색 결과 안에 "정정"이라고 표시된 문단이 있으면 그 정정을 따른다. 원문이 낡아 최신 공지와 \
                어긋날 때 붙여둔 것이므로, 원문 쪽 숫자를 인용하지 않는다. 같은 항목을 두고 자료가 엇갈리면 \
                항상 최신 공지가 FAQ·안내 페이지보다 우선한다.
                - 한도는 두 종류이며 섞어 쓰지 않는다: 구매(충전)한도 = 한 달에 새로 충전할 수 있는 금액, \
                보유한도 = 잔액으로 갖고 있을 수 있는 상한. 어느 쪽을 묻는지 구분해 답한다.
                ...
                따라서 "조회했다·검색해 보니"처럼 이미 조회한 것처럼 말하지 말고, 그 화면에서 조회 버튼을 눌러 확인할 수 있다고 안내한다. \
                이때도 "카드"처럼 화면 요소를 단정하지 않는다 — 자동모드가 켜져 있으면 카드가 뜨지 않는다.
