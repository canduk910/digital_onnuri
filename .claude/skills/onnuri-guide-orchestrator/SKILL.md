...
3. **착수 목록이 이미 있다.** 무엇을 할지 모델이 스스로 고르지 않는다 — 조건 1·2 만으로 들어가면 범위를 리더가 정하게 되고, 사용자는 자기가 승인한 적 없는 작업 결과를 아침에 받는다.

셋 중 하나라도 없으면 단발로 처리한다. 애매하면 착수 목록과 종료 조건을 한 번 확인하고 시작한다 — 이 한 번의 확인이 긴 호흡 전체의 승인을 대신한다.

### 착수 (첫 푸시가 나가기 전에 끝낸다)

- 착수 목록과 순서를 사용자에게 보여 주고 확정한다.
...
- **야간 배치 창(23:30~01:30)** — 경로를 가리지 않고 푸시하지 않는다. 게이트가 끝나기 전에 배치가 그 커밋을 당겨 갈 수 있다.
