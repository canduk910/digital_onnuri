...
(2026-09-05 에 참고용 실측값을 괄호로 적어 뒀는데 **하루 만에 낡았다** — 채록이 184에서
 189가 됐다. 그래서 지웠다. 이 문단이 말하는 바로 그 일이 그 줄에 일어났다.)
...
**고정 숫자로 적지 않는다.** 가맹점은 야간 배치가 매일 새로 수집하므로 값이 조금씩 움직인다.
고정값으로 두면 정상을 실패로 읽거나, 몇 번 겪은 뒤 아예 안 보게 된다(`backend/DEPLOY.md` 와
같은 원칙이다 — 그쪽과 이쪽이 다른 답을 들고 있으면 대조하는 사람이 어느 쪽을 믿느냐로 갈린다).

| 질의 | 범위 | 2026-09-06 실측 |
|---|---|---|
| 서울 전체 | 29,000~30,500 | 29,803 |
| 인천 / 경기 / 부산 전체 | 7,200~7,600 / 29,500~31,000 / 12,500~13,000 | 7,356 / 30,193 / 12,759 |
| 서울 강남구 | 740~790 | 761 |
| 서울 강남구 개포동 | 140~155 | 145 |
| 경기 수원시 팔달구 | 1,230~1,290 | 1,259 |
| 부산 해운대구 | 665~710 | 685 |
| 경기 brand=GS더프레시 | 6~12 | 8 |
| bounds(37.49~37.51, 127.02~127.07) | 550~600 | 569 |
| 전국(region 생략) brand GS25 | 350~390 | 368 |
| facets 키 | 구조를 본다 — `cat`·`brand`·`mtype` 세 키가 있는가 | 세 키 |

보아야 할 것은 **자릿수가 맞는가**와 **0 이 아닌가**다. 크게 어긋나면(예: 절반) 적재가 덜
됐거나 필터가 깨진 것이다. 정확한 회귀 대조는 이 값이 아니라 배치의 ±20% 가드(단계 A3)와
카나리아(단계 E)가 한다.

**팔달구를 셀 때 `si` 를 빼지 마라.** 2026-09-06 실측: `gu=팔달구` 만으로 세면 1,260 이고
`si=수원시` 를 함께 주면 1,259 다. 차이 한 건은 데이터 이상이다 — `평촌왕소금구이` 의 주소는
