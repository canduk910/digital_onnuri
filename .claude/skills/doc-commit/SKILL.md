...
**푸시는 곧 배포다.** 무엇이 자동으로 나가는지, 푸시 전에 종료 코드로 확인할 것, **승인을 받아야 하는 것**, **시각 때문에 미루는 것**은 모두 오케스트레이터 Phase 0-C 의 「푸시 게이트」·「푸시하지 않고 승인을 받는 것」·「푸시를 미루는 것」 세 절이 정본이다. 자율 실행이든 사용자가 시킨 것이든 같은 목록을 쓴다.

특히 **야간 배치 창(23:30~01:30)에는 커밋만 하고 푸시하지 않는다.** 승인을 기다리는 것이 아니라 시각을 기다리는 것이므로, 01:30이 지나면 쌓인 커밋을 그대로 푸시한다.
