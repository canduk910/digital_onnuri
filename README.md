...
| **가맹점 찾기(merchants)** | 79,800여 곳을 지역 계층·업종·브랜드 다중 필터로 검색. 네이버 지도 연동, 리스트↔지도 병행 — 상세 기능은 아래 |
| **온라인 사용처 찾기(online)** | 온라인 가맹 플랫폼 31곳 + 취급 물품종류·브랜드 태깅 검색 |
| **결제 방법(payment)** | 카드형·QR형 결제 단계, 전월 실적·선차감·잔액부족 처리 |
| **용어·유의사항(terms)** | 골목형상점가·SSM·선차감 등 용어 풀이와 결제 전 유의사항 |
| **버그 제보(report)** | 익명 오류 제보 게시판 — 처리상태(접수↔반영)는 아래 관리자 페이지에서 갱신 |
| **AI 챗봇** | 공식 출처 기반(RAG) 질의응답 + 페이지 이동·검색 실행. 전 페이지 플로팅 위젯 |

### 가맹점 찾기 상세

- **지도**: 뷰포트 자동 도트/클러스터 전환(3,000곳 기준, 클러스터 숫자=가맹점 수), 동일 건물 가맹점은 그룹 마커(개수 배지)로 묶고 클릭 시 그 위치 전체 목록 팝업, "이 지도 범위로 목록 보기" 토글(지도를 움직이면 리스트 자동 재검색)
- **상세 팝업 액션**: 네이버 지도 장소보기 · **길찾기** · **거리뷰** — 데이터의 좌표·주소로 바로 연결
- **거리뷰**: 지도 우상단 토글 또는 팝업에서 진입. 열려 있는 동안 메인 지도에 촬영 도로(파란 길)가 표시되고, 지도를 클릭하면 그 지점 거리뷰로 이동(현재 위치는 주황 원으로 동기화). PC는 드래그 이동·크기조절 플로팅 패널, 모바일은 하단 시트
- **가까운 순 정렬**: 내 위치 기준 거리순(서버 정렬·JSON 폴백 동일 수식) + 리스트에 거리 태그(m/km)
- **즐겨찾기·최근 본·공유**: 행의 ☆로 저장, "★ 저장" 모달에서 열람·해제, 위치 공유 링크(`?spot=…`) 복사 — 받은 사람은 그 가맹점 위치·팝업으로 바로 착지. 모두 브라우저에만 저장(서버 미전송)

### 공통 UI

- PC: 좌측 사이드바 접기(◀/❯), 화면 폭 슬라이더(최대 시 사이드바 접힘에 맞춰 자동 확장), 리스트↔지도 폭·높이 드래그 조절
- 모바일: 결과 표가 카드형으로 전환(이중 스크롤 없음), 지도 높이 뷰포트 비례, 44px 터치 타깃

### 운영 — 제보 관리자 페이지 (admin-report.html)

버그 제보의 처리상태(접수↔반영)를 토글하는 **운영자 전용** 페이지. 사이드바에 노출되지 않고 검색엔진 색인도 차단(noindex)된다.

- **진입**: ①관리자 비밀번호 로그인 — 서버가 검증(`POST /api/admin/login`)하고 성공 시 관리자 키를 발급, 무차별 대입 방지 rate limit(IP당 분 5회·일 30회) ②`?key=` 직접 진입(북마크용) — 첫 진입 시 키를 sessionStorage로 옮기고 URL에서 지운다
- **비밀값**: `APP_ADMIN_KEY`·`APP_ADMIN_PASSWORD`는 서버 `.env`에만 존재(저장소·문서 비노출). 미설정 시 해당 기능만 비활성(403) — 나머지 API 무영향
- 키 설정·상태 갱신 절차의 정본은 `backend/DEPLOY.md`
...
- **수집일 스탬프**: 배치가 갱신한 실제 수집일을 `/api/meta`로 노출하고 페이지의 "○○ 수집" 표기가 이를 따른다 — 화면의 날짜는 항상 실데이터 기준이다.
