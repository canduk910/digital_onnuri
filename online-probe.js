...
(function () {
  ...
  var REASON_LONG = {
    ...
    // 매장 목록 훑기(/store/onnuri)와 이 페이지의 상품명 색인(이 몰은 색인 대상이다).
    "no-static-search": "이 몰의 온누리 매장 화면에는 검색 기능이 없어 자동으로 확인할 수 없습니다. 링크로 들어가 온누리 매장 목록을 직접 훑어보거나, 이 페이지의 상품명 색인을 참고하세요."
  };
  /**
  ...
  /**
   * 상품명 색인 층(ADR-18 — 그때 이름은 "전일 색인"이었다) — 야간 배치가 모아 둔 상품명에서 찾은 결과.
   *
   * 실시간 층과 **섞지 않는다**. "수집 시점에 이 이름의 상품이 올라와 있었다"와 "지금 검색된다"는
   * 다른 주장이고, 한 목록에 넣으면 문구가 거짓이 된다. 그래서 별도 블록·별도 헤드라인·
   * 한 단계 약한 시각 위계로 두고, 상단에 `상품명 색인` eyebrow 를 박는다.
   * 상대 표현("전일"·"어제")은 쓰지 않는다 — 배치는 당일 00:30 에 돌고, 실패한 날에는
   * 사흘 전 것이 남는다. 시점은 응답의 실제 날짜로만 말한다(2026-09-04).
   *
  ...
  function indexBlock(idx, q) {
    ...
    return '<div class="pb-index">'
      + '<div class="pb-idx-eyebrow">상품명 색인</div>'
  ...
  function renderResult(mount, data, bridge) {
    ...
    mount.innerHTML =
      ...
      + (idxHtml
          ...
          ? '위 목록은 각 몰의 검색 결과를 그 자리에서 읽은 것이고, 상품명 색인은 야간 배치가 미리 모아 둔 상품명에서 찾아본 결과입니다. '
